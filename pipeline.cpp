...
class Assembler{
  public:
    ...
    string instructionToMachine(string inst){
        ...
        if(el.size()>1){//instruction is not just a label
            ...
            else if (el[0] == "addi" || el[0] == "xori" || el[0] == "andi" || el[0] == "ori")
            {
                ...
                bitset<12> b(index);//converting the labelIndex to a 5-bit binary string
    ...
    vector<int> ReadLocation(vector<string> el){
        vector<int> read;
    ...
    int WriteLocations(vector<string> el){
        for(int i=0;i<el.size();i++){
            if(el[i][0]=='$'){
                el[i].erase(el[i].begin());
            }
    ...
    vector<string> generateMachineCode(){
        ...
        for(int i=0;i<instructions.size();i++){
            ...
            int val=WriteLocations(temp);
            vector<int> reading;
            int k=instructions.size();
            if(i<k-1){
                bool cond=0;
                reading=ReadLocation(elements(instructions[i+1]));
                for(auto x: reading){
                    if(x==val){
            ...
            if(i<k-2){
                bool cond=0;
                reading=ReadLocation(elements(instructions[i+2]));
                for(auto x: reading){
                    if(x==val){
                        string vec="addi $31,$0,0";
                        instructions.insert(instructions.begin()+i+1,vec);
            ...
            if(i<k-3){
                bool cond=0;
                reading=ReadLocation(elements(instructions[i+3]));
                for(auto x: reading){
                    if(x==val){
                        string vec="addi $31,$0,0";
                        instructions.insert(instructions.begin()+i+1,vec);
